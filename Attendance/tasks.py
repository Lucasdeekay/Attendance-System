...
from Attendance.functions import get_spreadsheed_data_as_list_weekly_attendance, \
    get_spreadsheed_data_as_list_weekly_attendance_per_department
from Attendance.models import Password, Department, Staff
...
def mail_hods():
    ...
    if current_day.weekday() == 5:
        ...
        for department in departments:
            ...
            # Give the file created a name
            filename = f"Attendance For The Week for {department.department_name} ({days[-1]} - {days[0]}).xlsx"
            ...
            students_records = get_spreadsheed_data_as_list_weekly_attendance_per_department(days, department)
            ...
            message = EmailMessage(subject, msg, EMAIL_HOST_USER, [lecturer.person.email], )
            message.attach(filename, output.getvalue(), 'application/vnd.ms-excel')
            message.send()
